...
2. Apply `skills/audit_objective.md` to assess the objective.
3. Report all violations with severity.
4. Provide a rewrite if the objective fails the quick test.

## Context

- Apply `skills/audit_objective.md` strictly — every criterion must be evaluated.
- Apply `skills/execution_system.md` to verify the objective sits correctly at the top of the hierarchy.

## Rules

1. Evaluate all 9 criteria from `skills/audit_objective.md`. Never skip one.
